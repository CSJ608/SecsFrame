...
- 显式启用、会话关联且容量有界的运输故障快照分级导出。

当前切片不是原始网络抓包器；除显式启用的运输故障有界前缀外，不记录 TCP
...
连接必须先显式启用独立且容量有界的运输故障观测流；Trace 不自动订阅：
...
观测覆盖 <code>DecodeFailed</code>、<code>DiscardedByResync</code>、
<code>IncompleteFrameOverflow</code> 与 <code>IncompleteFrameTimeout</code>，
保留原始 transport Session ID、actor 状态、实际观测字节数和截断状态。
通道满时丢弃最旧项而不阻塞协议 actor；默认关闭时不创建通道。SecsFrame
对所有类型统一最多保留 8 KiB 前缀，快照可能为空，也可能包含四字节 HSMS
长度前缀；<code>ObservedByteCount</code> 和 <code>IsTruncated</code> 明确
说明该前缀是否完整。

<code>SecsTraceTransportFaultCaptureOptions</code> 同样要求明确选择
MetadataOnly、RedactedPayload 或 RawPayload。Trace 不会再次截断观测已经
保留的快照；超过调用方收紧后的限制立即失败。上游或 SecsFrame 已发生的
截断会原样保留。Redacted 范围相对快照起点，必须有序、非重叠、不越界并
在记录中全部为零。记录和公共观测都防御性复制字节。

独立 v2 信封使用十一个单空格分隔字段：
...
分类、保留快照长度、实际观测字节数、Complete/Truncated、脱敏范围和大写
十六进制快照。默认 codec 只允许 MetadataOnly；带快照的导入和导出必须
再次显式启用。信封不含异常或分片时序，也没有重放 API。v2 严格 codec
明确拒绝 <code>SecsFrame-TransportFaultTrace/1</code>，因为 v1 不含实际
字节数和完整性字段，不能可靠推断。
...
和 Body/记录/文本资源限制测试。运输故障信封另有四种错误、真实 TCP T8
前缀、旧 Session 过滤、8192/8193 字节边界、有界队列、三种分类、默认
拒绝、脱敏复核、畸形字段和快照/记录/文本资源限制测试。
