...
- [ ] 报警目录/启停/历史、远程命令和更完整的 GEM 状态/错误恢复。
- [ ] 使用授权 E30-0526 核对基础消息条件、数据项、应答值和状态转换。

当前报告和报警通知切片使用可替换工程 profile、动态
<code>SecsItem</code> 标识，并保留报警代码原始字节，不宣称已经核对
E30 的报告生命周期、报警代码位语义或应答枚举。下一优先级是远程命令，
再扩展报警管理以及更完整的 GEM 状态和错误恢复。
