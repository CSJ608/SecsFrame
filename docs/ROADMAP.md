...
下一优先级转向报警历史/批量控制。最小切片应先用授权 E30-0526 核对消息
条件、数据结构和代码语义，再确定可替换 profile 下的不可变历史模型、
Equipment 快照边界、Host 查询 API、严格向量和真实 TCP 证据；核对完成前
不固化新的默认消息对。自动恢复和命令目录/权限/调度继续保持后续评估项。
