...
- 本机 <code>SecsFrame.CommunicationDemo</code> 已形成连接、SML 发送、
  入站显式回复、会话内收藏、Linktest/Separate，以及筛选和两级脱敏导出的
  会话内诊断活动流；<code>SecsFrame.GuidedDemo</code> 已形成九步真实链路，
  覆盖核心 HSMS、GEM 工程基础、结构化脱敏 Trace 和真实 T3 诊断 Trace。
  两个项目的数据边界、运行方式和跨会话账本见 [Demo 接续](DEMOS.md)。
- <code>SecsFrame.DemoLauncher</code> 可从源码或发布包统一启动两个 Demo，
  限制为本机回环端点并在静态资源就绪后开放体验；独立 PowerShell 脚本和
  手动工作流生成 framework-dependent .NET 8 ZIP，发布构建不覆盖运行中的
  项目输出。Demo 测试已进入普通 Linux CI。
...
Demo 工作按 [Demo 体验与迭代接续](DEMOS.md) 独立分轮：D1 至 D4 已完成，
D5 正在进行。统一启动器、隔离发布构建、ZIP/手动工作流、发布后健康与静态
资源验证，以及页面键盘语义修正已经落地；下一接续动作只完成 390px 截图和
真实 Tab/动作后焦点浏览器验收，并处理由此发现的问题，不同时开启 D6。
任何新增落盘或分享能力仍须先定义数据分级；运输故障样本若加入固定脚本，
必须继续复用既有公共 API，且不能新增未经标准核对的消息语义。
