...
核心包不反向依赖它。
<code>SecsFrame.Trace</code> 依赖 SML 表示层和核心公共 API，处理已经解码
的数据消息，以及调用方显式提供的公共解码失败事件；它不订阅内部 transport
或创建第二个公共事件消费者。
...
- `SecsFrame.Trace` 依赖 `SecsFrame.Sml` 与 `SecsFrame`，重放必须回到
  ...
  元数据导出不得包含 Body 或隐式订阅任何连接流。故障样本只能从调用方
  已取得的解码失败事件显式创建，默认不得导入或导出 Body，也不得进入重放。
