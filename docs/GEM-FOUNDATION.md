...
<code>GemRemoteCommandEntryRegistration</code> 暴露精确标识并可释放。
同一结构相等标识不能重复注册。应用还可使用
...
处理器时请求仍立即失败，接受策略不会执行。接受策略、精确/回退选择和当前
状态在 Equipment 锁内按请求一起快照，并在锁外按策略、执行顺序调用；释放
注册只影响后续请求，已取得的快照会完成。策略或执行处理器的异常和取消
继续传播给事件循环，不会被折叠为失败应答。

当前切片只要求请求参数名和结果参数名各自唯一，不推断结果必须覆盖哪些
请求参数，也不内置任何通讯/在线状态矩阵。运行期目录只负责应用侧精确
分派，不新增 E30 消息、不提供 Host 目录查询，也不包含参数 Schema、权限、
排队、自动重试、幂等策略或执行历史。
...
- 远程命令必须设置 W-Bit 且 Secondary 消息对完全匹配；整体和参数结果码
  ...
  profile 失败值和空参数结果，且不得调用执行处理器。

畸形输入和提供器失败会作为异常返回应用事件循环；语义有效但引用未知变量
或报告的配置使用失败应答拒绝。本切片尚未实现自动 S9Fx、错误 Secondary、
自动重新建立、远程命令以外的业务消息状态门控、报警历史/批量控制、命令
目录/权限/调度或完整 GEM 错误恢复，这些行为不能从当前 API 推断为标准
合规。
...
优先/全局回退/释放恢复、无处理器拒绝、应用状态拒绝短路与同会话重试、
