...
Host 与 Equipment 还可显式调用 <code>RestoreCommunicationAsync</code>。
该方法先等待当前或下一次 HSMS Selected，再通过当前 profile 发起恰好一次
既有通讯建立事务；它不消费第二份事件流、不循环，也不合并并发调用。等待
或事务期间的取消只终止本次调用，不会在后续重选时迟到发送。对端拒绝、
会话在等待后再次失效或其他传输失败会直接传播，应用决定是否再次调用。

收到非 Selected 状态事件时，通讯状态、对端身份和在线状态仍先全部重置。
恢复成功只重新记录 <code>Communicating</code> 与
<code>PeerIdentity</code>，不会把 <code>OnlineState</code> 隐式恢复为
断线前状态；应用必须另行请求或执行在线转换。与所有 GEM 状态一样，正确
重置仍依赖应用把完整单消费者事件流交给 <code>TryDispatchAsync</code>。
...
后台通讯重建、报警历史/批量控制、完整命令权限/参数 Schema/调度或完整
...
重新建立拒绝时的状态/身份保持与恢复、Separate 后状态/身份清空、重选后的
显式单次恢复、等待取消后零迟到事务、恢复拒绝与显式重试、在线状态不隐式
恢复、在线转换接受/拒绝与状态保持、异构动态标识、报告定义、
