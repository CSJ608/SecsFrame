...
没有注册接受策略时命令直接进入执行处理器，保持此前行为。没有执行处理器
时请求仍立即失败，接受策略不会执行。两个处理器在 Equipment 锁内按请求
一起快照并在锁外按策略、执行顺序调用；释放注册只影响后续请求，已取得的
快照会完成。策略或执行处理器的异常和取消继续传播给事件循环，不会被折叠
为失败应答。

当前切片只要求请求参数名和结果参数名各自唯一，不推断结果必须覆盖哪些
请求参数，也不内置任何通讯/在线状态矩阵。本切片不包含命令目录、参数
Schema、权限、排队、自动重试、幂等策略或执行历史。
...
快照、单报警发送启停与未知标识拒绝、远程命令动态参数与结果、无处理器
拒绝、应用状态拒绝短路与同会话重试、时钟读写和 Linktest。它们是独立
工程验证，不是 SEMI 一致性认证。
