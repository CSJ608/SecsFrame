...
| GEM | SEMI E30-0526 | 通讯建立与双向应用接受策略、上下线、动态变量/常量、时钟、报告/事件、报警通知/目录/发送启停，以及带应用状态接受策略、运行期精确分派目录和逐项本地可用性的远程命令已有可替换 profile 的工程实现；规范性条件待核对 |
...
- E30：通讯建立、在线/离线、变量、事件报告、报警、远程命令和时钟能力
  ...
  控制码、应用通讯建立/在线转换策略、远程命令执行条件与状态门控、时间
  格式、状态转换与错误响应。
