namespace SecsFrame.Gem;

/// <summary>Describes one runtime alarm-catalog entry.</summary>
public sealed class GemAlarmDefinition
{
    /// <summary>Creates an alarm definition without interpreting its code.</summary>
    public GemAlarmDefinition(byte code, SecsItem alarmId, string text)
    {
        Code = code;
        AlarmId = alarmId ?? throw new ArgumentNullException(nameof(alarmId));
        Text = text ?? throw new ArgumentNullException(nameof(text));

        _ = SecsItem.Ascii(text);
    }

    /// <summary>Gets the exact alarm code byte.</summary>
    public byte Code { get; }

    /// <summary>Gets the exact dynamic alarm identifier.</summary>
    public SecsItem AlarmId { get; }

    /// <summary>Gets the seven-bit ASCII alarm text.</summary>
    public string Text { get; }
}
