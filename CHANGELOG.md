...
- Demo 页面增加跳到主要内容、焦点可见样式、区域/忙碌状态与进度/当前步骤
  语义；分步演示在每个动作完成后把键盘焦点移到实际结果标题，窄屏主要控件
  保持至少 44px 触达高度。
- 发布启动器为每个 Demo 使用各自内容根，并让健康检查同时验证静态资源，
  避免进程和健康端点正常但 CSS/Blazor 资源不可用。
...
- 增加 <code>SecsFrame.DemoLauncher</code>，可从源码或发布包统一启动两个
  Demo，仅接受本机回环 HTTP 地址，等待专用健康检查后打开实际体验页，并
  统一处理提前退出和关闭。
- 增加隔离项目输出的 Demo 发布脚本、Windows/Linux/macOS 启动脚本、包清单
  与手动 <code>demo-package</code> 工作流，生成 framework-dependent .NET 8
  ZIP 并执行发布后双应用启动验证；Demo 自动化测试同时进入普通 Linux CI。
...
- 增加手动和每周定时的 <code>session-soak</code> 工作流，固定 20 分钟程序
  上限、25 分钟作业上限、每周期 20 秒上限及 100000 轮次级上限；成功或
  失败均上传包含 commit、运行时、seed、故障序列和结果的 JSONL，保留
  14 天。
