...
| `SecsFrame.Trace` | 已解码消息、受限诊断与故障样本的导出、脱敏和受控重放 |
...
建立及应用接受策略、同会话显式重试、断线重选后的显式单次恢复、上下线与
应用转换策略、动态变量/常量、应用托管时钟、报告定义、事件链接和带
...
运输层互操作诊断另有默认关闭、容量有界的独立观测流；它覆盖解码失败、
重同步丢弃、未完成帧溢出和 T8 超时，保留错误来源 transport Session ID、
实际观测字节数、截断状态和最多 8 KiB 的前缀快照。Trace 默认仍只导出
元数据，快照捕获和 payload codec 必须分别显式开启。控制与运输故障观测
都和业务事件流独立；该包不会自行接管任何连接消费者，也不会复用原事务
标识。
Item 用法与边界见
